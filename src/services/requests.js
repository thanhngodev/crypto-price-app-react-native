...
export const getCoinMarketChartData = async (coinId) => {
    try {
...
export const getWatchlistedCoin = async (pageNumber = 1, coinId) => {
    try {
        const response = await axios.get(`https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&ids=${coinId}&order=market_cap_desc&per_page=50&page=${pageNumber}&sparkline=false&price_change_percentage=24h`)
        return response.data
    } catch (e) {
        console.log(e);
    }
